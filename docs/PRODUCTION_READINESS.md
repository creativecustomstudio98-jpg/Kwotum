...
| PR-07 | P1 / częściowo       | Resend, aplikacyjny outbox, pięciominutowy scheduler, rozdzielone sekrety, heartbeat i dokładnie jedna syntetyczna dostawa firmy działają produkcyjnie. DPA, niezależny alert probe i test v2 w realnych klientach pozostają otwarte.              | `docs/AUTH_EMAILS.md`, `docs/NOTIFICATION_OPERATIONS.md`, `apps/web/lib/notifications/worker.ts`     | Zatwierdzić DPA, podłączyć niezależny alert 503 → recovery oraz wykonać test białego HTML/text v2 w Gmailu, Apple Mail i Outlooku.      |
