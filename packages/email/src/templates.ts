...
function companyTemplateV2(input: NotificationTemplateInput): RenderedEmail {
  ...
  const contactDetails = [
    ...
    ...(preference.channel ? [{ label: "Preferowany kanał", value: preference.channel }] : []),
    ...(preference.window ? [{ label: "Preferowana pora", value: preference.window }] : []),
  ];
...
function contactPreference(input: NotificationTemplateInput): Readonly<{
  channel: string | null;
  html: string;
  lines: string;
  rows: string;
