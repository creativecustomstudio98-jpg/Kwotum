...
reset role;
create temporary table lead_pipeline_tenant_target as
select id as lead_id
from public.leads
where contact_email = 'klient@example.test';
grant select on lead_pipeline_tenant_target to authenticated;

do $$
begin
  if (select count(*) from lead_pipeline_tenant_target) <> 1 then
    raise exception 'tenant isolation target is missing';
  end if;
end;
$$;
...
begin
  ...
  begin
    perform public.change_lead_status(
      'aaaaaaaa-0000-4000-8000-000000000001',
      (select lead_id from lead_pipeline_tenant_target),
      'won'
    );
