...
import { enqueueWebhookTest } from "../../../../../../../../lib/webhooks/service";
import {
  mapWebhookError,
  webhookError,
  webhookJson,
...
export async function POST(
  ...
  if (!organizationId.success || !endpointId.success || !idempotencyKey.success) {
    return webhookError("INVALID_REQUEST", "Dane testu są nieprawidłowe.", 422, requestId);
  }
  try {
    return webhookJson(
      await enqueueWebhookTest(organizationId.data, endpointId.data, idempotencyKey.data),
      202,
      requestId,
    );
  } catch (error) {
