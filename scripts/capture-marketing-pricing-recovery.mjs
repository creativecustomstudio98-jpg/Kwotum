...
const baseUrl = process.env.MARKETING_PRICING_BASE_URL ?? "http://127.0.0.1:3100";
...
try {
  for (const viewport of viewports) {
    ...
    const response = await page.goto(`${baseUrl}/cennik?qa=pricing-recovery`, {
      waitUntil: "networkidle",
    });
    await page.addStyleTag({
      content:
        ".marketing-header { position: static !important; } html body .skip-link, nextjs-portal { display: none !important; visibility: hidden !important; opacity: 0 !important; }",
    });
    await page.locator('a[href="#main-content"]').evaluateAll((links) => {
      for (const link of links) link.remove();
    });

    const pricing = page.locator("[data-pricing-simple]");
    const finalCta = page.locator("[data-pricing-final-cta]");
    const metrics = await page.evaluate(() => {
      const pricingRegion = document.querySelector("[data-pricing-simple]");
      const finalRegion = document.querySelector("[data-pricing-final-cta]");
      const cards = [...document.querySelectorAll(".pricing-simple-card")];
      ...
      const actions = [...document.querySelectorAll(".pricing-simple-card__action")].map(
        (action) => {
          const bounds = action.getBoundingClientRect();
          return { height: bounds.height, width: bounds.width };
        },
      );
      const textSizes = [pricingRegion, finalRegion]
    ...
    await pricing.screenshot({
      animations: "disabled",
      path: join(outputDirectory, `cennik-plans-${viewport.label}.png`),
    });
    await finalCta.screenshot({
      animations: "disabled",
      path: join(outputDirectory, `cennik-cta-${viewport.label}.png`),
    });
